...
可以对json实现object嵌套，即一个json对象可以可以通过键值对
让值设为另一个json对象，而json对象又可以包含数组、字符串等其他类型的值

Value类中包含了对各种类型value的封装，包括：
1.
构造函数：可以根据不同的类型构造不同的value，包括null、bool、int32、int64、double、string、array、object
2.
类型判断函数：可以判断value的类型，包括isNull、isBool、isInt32、isInt64、isDouble、isString、isArray、isObject
3.
取值函数：可以根据value的类型，获取对应的value，包括getBool、getInt32、getDouble、getArray、getObject、getString、getInt64、getStringView
4. 运算符重载：可以对value进行赋值 还可以下标访问array   利用key访问object
5.
内存管理：value的内存管理采用AddRedCount，用引用计数管理内存，当引用计数为0时，才允许释放内存;
并利用该结构体存储string array object类型
...
public:
  ...
  bool isInt64() const {
    return type_ == ValueType::TYPE_INT64 || type_ == ValueType::TYPE_INT32;
  }
  ...
  // placement new 用于在已有的内存上构造对象 用法： new (pointer)
  // Type(arguments);
  Value &setNull() {
    this->~Value();
  ...
  constMemberIterator cbeginMember() const {
    assert(type_ == ValueType::TYPE_OBJECT);
    return o_->data.cbegin();
  }
  MemberIterator endMember() {
    assert(type_ == ValueType::TYPE_OBJECT);
    return o_->data.end();
  }
  constMemberIterator cendMember() const {
    assert(type_ == ValueType::TYPE_OBJECT);
    return o_->data.cend();
  }
...
private:
  ...
  struct AddRefCount {
    ...
    AddRefCount(Args &&... args)
        : refCount(1), data(std::forward<Args>(args)...) {}

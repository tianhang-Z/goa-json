...
Document的内部依赖于Reade::parse进行json解析 并自己实现了handler接口
...
public:
    ...
    bool StartObject(){
        ...
        stack_.emplace_back(value); // 仅在遇到 { 时, 将当前对象压入栈  ;  遇到 } 时, EndObject出栈
        return true;
    }
...
private:
    ...
    Value* addValue(Value&& value){   
        ...
        if(top.type()== ValueType::TYPE_ARRAY){
            top.value->addValue(std::move(value));   //利用Value的addValue接口添加到数组中
            top.valueCount++;

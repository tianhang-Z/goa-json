...
inline unsigned countDigits(unsigned n) {
  ...
  //
  // 下面是求解n的十进制位数的数学近似方法  更高效
  // 因为__builtin_clz是gcc是硬件指令  效率很高
...
template <typename T> inline unsigned itoa_(T val, char *buf) {
...
template <typename WriteStream,
          typename =
              std::enable_if_t<std::is_same_v<WriteStream, StringWriteStream> ||
...
public:
  ...
  bool Double(double d) {
    ...
    if (std::isinf(d)) {
      ...
      auto it = std::find_if(buf, buf + n,
                             [](char c) { return c == '.'; }); // find '.'
      if (it == buf + n) {
        strcat(buf, ".0");
      }
    }
  ...
  /*
  在 JSON 中，控制字符（例如 ASCII 码小于 32 的字符）需要通过 Unicode
  转义序列进行表示。 JSON 使用 \u 后跟四位十六进制数来表示这些字符。
  */
